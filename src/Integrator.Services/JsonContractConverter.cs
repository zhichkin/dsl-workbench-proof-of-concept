...
{
    public sealed class JsonContractConverter : JsonConverter<DummyAssembly>
    {
        private readonly IReferenceResolver _resolver;
        private readonly ISerializationBinder _binder;
        public JsonContractConverter(ISerializationBinder binder, IReferenceResolver resolver) : base()
        {
            _binder = binder;
            _resolver = resolver;
        }
        public override bool CanConvert(Type typeToConvert)
        {
            ...
            return typeToConvert.IsAssignableFrom(typeof(DummyAssembly));
        }
        ...
        private void WriteProperty(Utf8JsonWriter writer, DummyAssembly source, PropertyInfo info, JsonSerializerOptions options)
        {
            if (info.IsOptional())
            {
        ...
        private void WriteArray(Utf8JsonWriter writer, IEnumerable list, JsonSerializerOptions options)
        {
            ...
            {
                if (enumerator.Current is DummyAssembly entity)
                {
        ...
        public override DummyAssembly Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
