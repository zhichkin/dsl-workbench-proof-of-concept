...
namespace OneCSharp.Metadata.Services
{
    public sealed class SQLHelper
    {
        ...
        public void Load(Domain infoBase)
        {
            foreach (Namespace ns in infoBase.Namespaces)
            {
                foreach (var item in ns.Entities)
                {
                    GetSQLMetadata((Entity)item);
                }
            }
        }
        private void GetSQLMetadata(Entity metaObject)
        {
            ReadSQLMetadata(metaObject);
            foreach (var nestedObject in metaObject.NestedEntities)
            {
                ReadSQLMetadata((Entity)nestedObject);
            }
        }
        private void ReadSQLMetadata(Entity metaObject)
        {
            ...
            foreach (var property in metaObject.Properties)
            {
                ...
                {
                    AddOCSField((Property)property, field, indexInfo);
                    field.IsFound = true;
                }
            }
            ...
            {
                AddOCSProperty(metaObject, field, indexInfo, position);
                position++;
                //field.IsFound = true; СтандартныеРеквизиты
            }
        }
        private void AddOCSField(Property property, SqlFieldInfo info, ClusteredIndexInfo indexInfo)
        {
        ...
        private void AddOCSProperty(Entity metaObject, SqlFieldInfo info, ClusteredIndexInfo indexInfo, int position)
        {
            ...
            AddOCSField(property, info, indexInfo);
        ...
        private void DefineOCSFieldPurpose(Field field)
        {
